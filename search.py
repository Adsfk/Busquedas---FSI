...
def graph_search(problem, fringe):
    ...
    while fringe:
        node = fringe.pop()
        problem.visited=problem.visited+1
        if problem.goal_test(node.state):
            print("Generados:%i " % problem.generated)
            print("Visitados:%i " % problem.visited)
            return node
        if node.state not in closed:
            closed[node.state] = True
            fringe.extend(node.expand(problem))
            problem.generated=problem.generated+len(node.expand(problem))
    return None
...
def heurist_first_graph_search(problem):
    return graph_search(problem, HeurisQueue(problem))
